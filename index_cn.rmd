...
```{r load packages, echo = FALSE, error = FALSE, message = FALSE, warning = FALSE, include = FALSE}
...
- <p class="fragment highlight-blue">????????????  </p>
      * <h4>??????,??????,?????? </h4>
...
- [?????????????????? - ??????????????????](https://ericoll.internal.ericsson.com/sites/Data_Vizalytics/Documents/MachineLearning/ML_workshop/ML_intro/ML_intro_PA14.pptx)

- ???????????? - <img src="figs/github-logo.png" 
...
- RStudio?????? <img src="figs/RStudio-Logo-Blue-Gradient.png" 
	height="50px" style="background-color:transparent; border:0px; box-shadow:none;"/>  

- MOOC (???????????????????????????) <img src="figs/MOOC_logo.svg" 
	height="50px" style="background-color:transparent; border:0px; box-shadow:none;"/>  
...
- Databricks?????? <img src="figs/databricks_logoTM_1200px.png" 
	height="50px" style="background-color:transparent; border:0px; box-shadow:none;"/>  

- ???????????? - Goodfellow Ian, Bengio Yoshua, Courville Aaron <img src="figs/deep_learning_goodfellow.jpg" 
...
## ?????? {data-background="figs/ML_five.svg"}

<div  class="fragment" style='float:left;width:50%'>

<ul><li class="fragment">????????????</li><br>
<li class="fragment">????????????????????????</li><br>
<li class="fragment">???????????????????????????</li><br>
<li class="fragment">??????????????????</li><br>
<li class="fragment">????????????????????????</li><br></ul>  
</div>
...
<figcaption><small>??????????????????</small></figcaption><br>
...
<figcaption> <small>??????????????????:??????????????????????????????</small> </figcaption>
<small>?????????: AT&T ??????^[AT&T ??????] </small>
<br>

<a href="http://scs.ryerson.ca/~aharley/vis/conv/" data-preview-link>- ???????????????????????????????????? ^[Convolution Neural Networks Viz http://scs.ryerson.ca/~aharley/vis/conv/]
...
<figcaption> <small>?????????????????????????????????</small> </figcaption></center>
...
<img src="figs/ML_learning_from_data.svg" style="float: left; border:0px; box-shadow:none; width: 70%; margin-right: 1%; margin-bottom: 0.5em"/><br><figcaption> <small>??????????????????^[Yaser S. Abu-Mostafa; ??????????????????]</small> </figcaption>
...
<h6>- ???????????????????????????????????????????????????</h6>
<h6>- ???????????????????????????????????????</h6>
<h6>- ????????????;  $x^k$, $\sqrt{x}$, $\log(x)$, $e^x$, $\Vert x \Vert$ </h6>
